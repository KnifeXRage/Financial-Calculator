...
@onready var simple_i_btn: CheckButton = $"../SimpleI_Btn"
@onready var clear_btn: Button = $Clear_Btn

@onready var la: LineEdit = $"../LA_Container/LA"
@onready var roi: LineEdit = $"../ROI_Container/ROI"
@onready var ly: LineEdit = $"../LY_Container/LY"
...
func hisBtnChar(state: bool) -> int:
	if state:
		return 3
	return 5
...
func createHistory(currDT: String = getDT(),amount: String = la.text,percent: String = roi.text,years: String = ly.text,btnPressed: bool = simple_i_btn.button_pressed) -> void:
	currChildCount += 1
	...
	box.setValues(removeSecondsFromDT(currDT),float(amount),float(percent),float(years),hisBtnChar(btnPressed),btnPressed)
...
func saveHistory() -> void:
	SaveLoad.save_data(currData,"EMI")

func loadHistory() -> void:
	currChildCount = 0
	var loaded: Dictionary = SaveLoad.load_data("EMI")
	if !(loaded == {}):
		enable(history_scroll)
		disable(no_history_text)
		var keys: Array = loaded.keys()
		for i in range(len(keys)):
			createHistory(keys[i],loaded[keys[i]]["amount"],loaded[keys[i]]["percent"],loaded[keys[i]]["years"],loaded[keys[i]]["btnPressed"])
			currChildCount += 1
