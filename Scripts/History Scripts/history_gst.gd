...
@onready var inclusive_btn: CheckButton = $"../Inclusive_Btn"
@onready var clear_btn: Button = $Clear_Btn

@onready var price: LineEdit = $"../Price_Container/Price"
@onready var custom_slab: LineEdit = $"../Slab_Container/Custom_Slab"
...
func createHistory(currDT: String = getDT(),amount: String = price.text,percent: String = custom_slab.text,btnPressed: bool = inclusive_btn.button_pressed) -> void:
	currChildCount += 1
	...
	box.setValues(removeSecondsFromDT(currDT),float(amount),float(percent),float(0),hisBtnChar(btnPressed),btnPressed)
...
func saveHistory() -> void:
	SaveLoad.save_data(currData,"GST")

func loadHistory() -> void:
	currChildCount = 0
	var loaded: Dictionary = SaveLoad.load_data("GST")
	if !(loaded == {}):
		enable(history_scroll)
		disable(no_history_text)
		var keys: Array = loaded.keys()
		for i in range(len(keys)):
			createHistory(keys[i],loaded[keys[i]]["amount"],loaded[keys[i]]["percent"],loaded[keys[i]]["btnPressed"])
			currChildCount += 1
