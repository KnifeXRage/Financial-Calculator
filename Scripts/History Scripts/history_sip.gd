...
@onready var lumpsum_btn: CheckButton = $"../Lumpsum_Btn"
@onready var clear_btn: Button = $Clear_Btn

@onready var mi: LineEdit = $"../MI_Container/MI"
@onready var roi: LineEdit = $"../ROI_Container/ROI"
@onready var tp: LineEdit = $"../TP_Container/TP"
...
func createHistory(currDT: String = getDT(),amount: String = mi.text,percent: String = roi.text,years: String = tp.text,btnPressed: bool = lumpsum_btn.button_pressed) -> void:
	currChildCount += 1
...
func saveHistory() -> void:
	SaveLoad.save_data(currData,"SIP")

func loadHistory() -> void:
	currChildCount = 0
	var loaded: Dictionary = SaveLoad.load_data("SIP")
	if !(loaded == {}):
		enable(history_scroll)
		disable(no_history_text)
